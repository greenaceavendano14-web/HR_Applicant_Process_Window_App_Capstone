...
namespace HRApplicantSystem
{
    public partial class ScreeningForm : Form
    {
        ...
        private Label lblQualificationHeader, lblRemarksHeader;
        ...
        private void InitializeComponent()
        {
            ...
            // Qualifications checklist group
        ...
        private void LoadExistingScreening()
        {
            try
            {
                using (MySqlConnection conn = new MySqlConnection(_connectionString))
                {
                    conn.Open();
                    string query = @"
                        SELECT IsQualified, Remarks, ScreenedDate
                        FROM ScreeningResults
                        WHERE ApplicationID = @AppID
                        ORDER BY ScreenedDate DESC LIMIT 1";
                    ...
                    if (reader.Read())
                    {
                        bool isQualified = Convert.ToBoolean(reader["IsQualified"]);
                        rbQualified.Checked = isQualified;
                        rbNotQualified.Checked = !isQualified;
                        txtRemarks.Text = reader["Remarks"].ToString();
                        lblExistingResult.Text = $"Previous screening on {reader["ScreenedDate"]:MM/dd/yyyy hh:mm tt}";
                    }
                }
            }
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(_connectionString))
                {
                    ...
                    insertCmd.Parameters.AddWithValue("@IsQualified", isQualified ? 1 : 0);
                    ...
                    string histQuery = @"
                        INSERT INTO ApplicationStatusHistory (ApplicationID, Status, ChangedByUserID, Remarks, DateChanged)
                        VALUES (@AppID, @Status, @UserID, @Remarks, NOW())";
            ...
            {
                MessageBox.Show("Error saving screening: " + ex.Message, "Error",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    }
