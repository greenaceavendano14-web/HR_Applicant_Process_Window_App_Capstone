        ...
        private void LoadApplicantData()
        {
            try
            {
                using (MySqlConnection conn = new MySqlConnection(_connectionString))
                {
                    ...
                    // Load application + applicant info
                    string query = @"
                        SELECT a.FirstName, a.LastName, a.Email, a.Phone, a.Address,
                               a.Education, a.Skills, a.WorkExperience,
                    ...
                    // Load documents
                    string docQuery = @"
                        SELECT rt.RequirementName AS DocType, ad.Status, ad.Remarks, ad.SubmittedDate
                    ...
                    MySqlDataReader docReader = docCmd.ExecuteReader();
                    ...
                    // Load status history
                    string histQuery = @"
                        SELECT ash.Status, CONCAT(u.FirstName,' ',u.LastName) AS ChangedBy,
                               ash.Remarks, ash.DateChanged
                        FROM ApplicationStatusHistory ash
                        LEFT JOIN Users u ON ash.ChangedByUserID = u.UserID
                        WHERE ash.ApplicationID = @AppID
                        ORDER BY ash.DateChanged ASC";
            ...
            {
                MessageBox.Show("Error loading applicant data: " + ex.Message, "Error",
        ...
        private void SaveScreeningResult(bool isQualified)
        {
            try
            {
                using (MySqlConnection conn = new MySqlConnection(_connectionString))
                {
                    conn.Open();
                    string query = @"
                        INSERT INTO ScreeningResults (ApplicationID, IsQualified, Remarks, ScreenedByUserID, ScreenedDate)
                        VALUES (@AppID, @IsQualified, @Remarks, @UserID, NOW())
                        ON DUPLICATE KEY UPDATE
                            IsQualified = @IsQualified, Remarks = @Remarks,
                            ScreenedByUserID = @UserID, ScreenedDate = NOW()";

                    MySqlCommand cmd = new MySqlCommand(query, conn);
                    cmd.Parameters.AddWithValue("@AppID", _applicationId);
                    cmd.Parameters.AddWithValue("@IsQualified", isQualified ? 1 : 0);
            ...
            {
                MessageBox.Show("Error saving screening result: " + ex.Message, "Error",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void UpdateApplicationStatus(string newStatus, string remarks)
        {
            try
            {
                using (MySqlConnection conn = new MySqlConnection(_connectionString))
                {
                    ...
                    string updateQuery = "UPDATE Applications SET Status = @Status WHERE ApplicationID = @AppID";
