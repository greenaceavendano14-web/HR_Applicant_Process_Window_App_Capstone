    ...
    public partial class InterviewEvaluationForm : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            for (int i = 0; i < categories.Length; i++)
            {
                ...
                scoreBar.Value        = 5;
                int barIdx            = i;
                nuds[i].ValueChanged += (s, e) => { scoreBar.Value = (int)nuds[barIdx].Value; };
            ...
            txtRecommendation.PlaceholderText =
                "e.g. Recommend for final interview, Endorse for technical assessment...";
        ...
        private void LoadInterviewInfo()
        {
            try
            {
                ...
                // InterviewSchedules columns: ScheduleID, ApplicationID, InterviewTypeID,
                // InterviewerUserID, ScheduledDate, ScheduledTime, Mode, Location, MeetingLink, Status
                string query = @"
                ...
                using (MySqlCommand cmd = new MySqlCommand(query, conn))
                {
                    ...
                    using (MySqlDataReader reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            ...
                            // Combine ScheduledDate + ScheduledTime into a display string
                            ...
                            lblInterviewDate.Text = "Date: " + combined.ToString("MM/dd/yyyy hh:mm tt");
        ...
        private void LoadExistingEvaluation()
        {
            try
            {
                ...
                // InterviewEvaluations columns: EvaluationID, ScheduleID, EvaluatedByUserID,
                // Score, Result ENUM('Pass','Fail','Pending'), Remarks, Recommendation, EvaluatedAt
                // Score is a single DECIMAL(5,2) — we store the average; individual scores are not
                // separate columns in the DB schema. We load what we have.
                string query = @"
                    SELECT Score, Result, Remarks, Recommendation, EvaluatedAt
                ...
                using (MySqlCommand cmd = new MySqlCommand(query, conn))
                {
                    ...
                    using (MySqlDataReader reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            // Spread a single saved score back across all categories
                            if (reader["Score"] != DBNull.Value)
                            {
                                decimal savedAvg = Convert.ToDecimal(reader["Score"]);
                                decimal perCat   = Math.Min(10, Math.Max(1, Math.Round(savedAvg)));
                                nudCommunication.Value  = perCat;
                                nudTechnical.Value      = perCat;
                                nudProblemSolving.Value = perCat;
                                nudProfessionalism.Value = perCat;
                                nudCulturalFit.Value    = perCat;
                            }
                            ...
                            lblExistingNote.Text    =
                                $"Evaluation last saved on {reader["EvaluatedAt"]:MM/dd/yyyy hh:mm tt}";
                        }
                    }
                }
            }
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            string result    = rbPass.Checked ? "Pass" : rbFail.Checked ? "Fail" : "Pending";
            string newStatus = rbPass.Checked ? "For Final Review"
                             : rbFail.Checked ? "Rejected"
                             : "For Assessment";

            if (MessageBox.Show(
                $"Save evaluation with result: {result}?\nApplication will move to: {newStatus}",
                "Confirm", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
                return;

            // Calculate average score from the five NUDs
            int    total    = (int)(nudCommunication.Value + nudTechnical.Value +
                                    nudProblemSolving.Value + nudProfessionalism.Value +
                                    nudCulturalFit.Value);
            decimal avgScore = total / 5.0m;
            ...
            try
            {
                ...
                // Schema has a single Score column (DECIMAL 5,2) — we store the average.
                string upsertQuery = @"
                    INSERT INTO InterviewEvaluations
                        (ScheduleID, EvaluatedByUserID, Score, Result, Remarks, Recommendation, EvaluatedAt)
                    VALUES
                        (@SchedID, @UserID, @Score, @Result, @Remarks, @Rec, NOW())
                ...
                using (MySqlCommand cmd = new MySqlCommand(upsertQuery, conn))
                {
                    ...
                    cmd.Parameters.AddWithValue("@Score",   avgScore);
                ...
                // Mark interview schedule as Completed
                using (MySqlCommand schedCmd = new MySqlCommand(
                    "UPDATE InterviewSchedules SET Status = 'Completed' WHERE ScheduleID = @SchedID", conn))
                {
                    schedCmd.Parameters.AddWithValue("@SchedID", _interviewScheduleId);
                ...
                using (MySqlCommand histCmd = new MySqlCommand(histQuery, conn))
                {
                    ...
                    histCmd.Parameters.AddWithValue("@Remarks",
                        $"Interview Result: {result}. {txtRemarks.Text.Trim()}");
                    histCmd.ExecuteNonQuery();
                }

                MessageBox.Show("Interview evaluation saved successfully!", "Saved",
